...
int pid, getPID(char bm[64]), count, ipid, oid, scwz, location,Detect;
float px,py;
unsigned long Starting_address = 0x0000000000000000;
unsigned long End_Address = 0x7fffffffffffffff;
...
int getutf(unsigned long addr)
{
	int var = 0;
    if (Data_type == 1) {
      vm_readv(addr, &var, 28);
    }else {
      syscall(SYS_pread64,处理内存,&var,28,addr);
    }
	return (var);
}
...
{
    /*
	if ((addr>=Starting_address && addr<=End_Address) || addr == 0){
	    Detect++;
	    printf("Detect次数：%d\n",Detect);
        return 0;
    }*/
...
void writedword(unsigned long addr, int data)
{
    if (Data_type == 1) {
      ...
      syscall(SYS_pwrite64,处理内存,&data,4,addr);
    }
} 
